...
// только до, но и после запуска кофеварки, то есть чтобы функцию onReady можно
// было изменить в любой момент до её срабатывания.
...
function CoffeeMachine(power, capacity) {
   ...
   this.run = function() {
      setTimeout(function(){return onReady()}, getTimeToBoil());
   };

   this.setOnReady = function (onReadyFunction) {
...
coffeeMachine.setOnReady(function() {
   var amount = coffeeMachine.getWaterAmount();
   alert( 'Ляляляжужужу: ' + amount + 'мл' ); // Кофе готов: 150 мл
});